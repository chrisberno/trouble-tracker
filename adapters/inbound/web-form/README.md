...
No `email` and no `company` field. CEO defaults locked 2026-05-03 — keeps
the form 4 fields wide, matching the legacy form's user workflow. See
"Known limitation" below for the consequence.
...
## 6. Known limitation — no email field

Without an email field on the form, `pp-client.ensureCustomer` cannot
deduplicate by email (Phase 1's 409-handling path keys on
`/api/contacts/search/{email}`). Each form submission therefore creates
a new PP.app customer record. Acceptable for Phase 2; revisit in Phase 4
when the customer feedback loop work would benefit from dedup.
